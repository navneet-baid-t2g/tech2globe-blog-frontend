...
export async function generateStaticParams() {
    // Fetch all authors
    const authorsRes = await fetch(`${process.env.API_BASE_PATH}/authors?limit=1000`);
    if (!authorsRes.ok) {
        return [];
    }
    ...
    for (const author of authors) {
        ...
        const postsRes = await fetch(`${process.env.API_BASE_PATH}/posts/author/${authorId}?page=1&limit=20`);
...
export default async function AuthorPage({
    ...
    return (
        <Layout headerStyle={1} footerStyle={1}>
            <BlogArea1 posts={posts} totalPages={totalPages} currentPage={currentPage} authorId={author_id} />
            <BlogCategories1 />
            <CtaArea1 />
        </Layout>
    );

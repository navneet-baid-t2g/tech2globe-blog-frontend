...
const Sidebar: React.FC<SidebarProps> = ({ author, showAuthor }) => {
    ...
    useEffect(() => {
        const fetchRecentPosts = async () => {
            try {
                ...
                setRecentPosts(json.data.posts.slice(0, 3));
            } catch (error) {
        ...
        <div className="blog1-sidebar-area mt-40 ml-30 sm:ml-0 md:ml-0 md:mt-30 sm:mt-30">
            ...
            {loadingPosts ? (
                <div className="sidebar-details-widget_1 _recent-posts mt-40">
                    <h3>Recent Post</h3>
                    {[...Array(3)].map((_, i) => (
                        <div className="blog1-recent-box mt-16 d-flex align-items-start gap-3" key={i}>
                            <div className="recent-thumb placeholder-glow flex-shrink-0" style={{ width: '82px' }}>
                                <div
                            ...
                            <div className="heading placeholder-glow w-100">
                                <div
                                    className="placeholder w-50 mb-2"
                                    style={{
                                        height: '12px',
                                        backgroundColor: '#4f5966',
                                        borderRadius: '4px',
                ...
                showAuthor && author && (
                    <div className="sidebar-details-widget_1 _author-intro mt-40">
                        <div className="sidebar-author-thumb text-center">
                            ...
                            <h4><Link className='text-dark' href={`/author/${author.id}`}>{author.displayName}</Link></h4>
            ...
            <div className="sidebar-details-widget_1 instagram-feed mt-40">
                    ...
                    <div className="row g-3">
                        {posts.map((post, index) => (
                            <div className="col-md-4 col-sm-6 col-12" key={post.id}>
                                <div className={`feed-single position-relative ${index >= 3 ? 'mt-3' : ''}`}>
                                    <div className="card border-0 overflow-hidden insta-card">
                                        <div className="position-relative">
                                            ...
                                            <Link href={post.url} target="_blank" className="stretched-link insta-link" aria-label="View on Instagram"></Link>
                                            ...
                                            <div className="position-absolute bottom-0 start-0 w-100 py-2 px-3 d-flex justify-content-between align-items-center bg-dark bg-opacity-50 text-white small stats-overlay">
                                                <span>
                                                    <i className="fa-solid fa-heart me-1"></i> {post.likesCount}
                                                </span>
                                                <span>
                                                    <i className="fa-solid fa-comment me-1"></i> {post.commentsCount}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                ...
                <Link href="https://www.instagram.com/tech2globeweb" className="insta-btn">View on Instagram <span><i className="fa-regular fa-arrow-right" /></span></Link>
            </div>
        </div >)
}
